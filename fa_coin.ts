...
import {FAUCET_URL, NODE_URL, PRIVATE_KEY} from "./common";
import {
  AptosAccount,
  CustomEndpoints,
  FaucetClient,
...
async function main() {
  ...
  const customEndpoints: CustomEndpoints = {
    fullnodeUrl: NODE_URL,
    indexerUrl: NODE_URL /* not used */
  }
  const client = new FACoinClient(customEndpoints);
  const fungibleAssetClient = new FungibleAssetClient(client);
  const faucetClient = new FaucetClient(NODE_URL, FAUCET_URL);

  // Create two accounts, Alice and Bob, and fund Alice but not Bob
  // const alice = new AptosAccount();
  const alicePkBytes: Uint8Array = HexString.ensure(PRIVATE_KEY).toUint8Array()
  const alice: AptosAccount = new AptosAccount(alicePkBytes)
